...
$('#register-form').addEventListener('submit', async e=>{
  ...
  try{
    ...
    await db.ref('users/' + uid).set({ email, role: 'pending', createdAt: firebase.database.ServerValue.TIMESTAMP, cryptoSalt: salt });
...
$('#login-form').addEventListener('submit', async e=>{
  ...
  try{
    ...
    const superAdmin = await isSuperAdmin(userCred.user);
    if(superAdmin){
