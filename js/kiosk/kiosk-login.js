...
async function loadKiosksForSelection() {
  try {
    ...
    if (!userProfile || userProfile.role !== 'approved') {
      ...
      setTimeout(() => { window.location.href = (window.WAITLESS_KIOSK_ROUTES && window.WAITLESS_KIOSK_ROUTES.login) || 'login.html'; }, 2000);
      return;
    }
...
async function submitPin() {
  ...
  try {
    ...
    try {
      ...
      setTimeout(() => {
        window.location.href = (window.WAITLESS_KIOSK_ROUTES && window.WAITLESS_KIOSK_ROUTES.interface) || 'interface.html';
      }, 500);
    } catch (err) {
